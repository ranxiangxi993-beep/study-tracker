// 选句原则：聚焦"鼓励学习、坚持、积极向上"——
// 已剔除写景、归隐、爱情、伤感、饮酒、战争/殉国等与学习无关的篇目，并补入若干劝学名句。
const QUOTES = [
  // === 先秦经典 · 劝学修身 ===
  '日拱一卒，功不唐捐',
  '不积跬步，无以至千里；不积小流，无以成江海',
  '锲而舍之，朽木不折；锲而不舍，金石可镂',
  '路漫漫其修远兮，吾将上下而求索',
  '亦余心之所善兮，虽九死其犹未悔',
  '天行健，君子以自强不息',
  '地势坤，君子以厚德载物',
  '学而时习之，不亦说乎',
  '温故而知新，可以为师矣',
  '学而不思则罔，思而不学则殆',
  '知之者不如好之者，好之者不如乐之者',
  '三人行，必有我师焉',
  '见贤思齐焉，见不贤而内自省也',
  '岁寒，然后知松柏之后凋也',
  '士不可以不弘毅，任重而道远',
  '逝者如斯夫，不舍昼夜',
  '譬如为山，未成一篑，止，吾止也；譬如平地，虽覆一篑，进，吾往也',
  '三军可夺帅也，匹夫不可夺志也',
  '敏而好学，不耻下问',
  '发愤忘食，乐以忘忧，不知老之将至',
  '默而识之，学而不厌，诲人不倦',
  '欲速则不达，见小利则大事不成',
  '工欲善其事，必先利其器',
  '人无远虑，必有近忧',
  '不怨天，不尤人，下学而上达',
  '知者不惑，仁者不忧，勇者不惧',
  '故天将降大任于是人也，必先苦其心志，劳其筋骨',
  '生于忧患，死于安乐',
  '富贵不能淫，贫贱不能移，威武不能屈',
  '虽千万人，吾往矣',
  '尽信书，则不如无书',
  '人有不为也，而后可以有为',
  '自暴者不可与有言也，自弃者不可与有为也',
  '道虽迩，不行不至；事虽小，不为不成',
  '青，取之于蓝而青于蓝；冰，水为之而寒于水',
  '吾尝终日而思矣，不如须臾之所学也',
  '积土成山，风雨兴焉；积水成渊，蛟龙生焉',
  '骐骥一跃，不能十步；驽马十驾，功在不舍',
  '君子生非异也，善假于物也',
  '蓬生麻中，不扶而直；白沙在涅，与之俱黑',
  '合抱之木，生于毫末；九层之台，起于累土；千里之行，始于足下',
  '天下难事，必作于易；天下大事，必作于细',
  '为学日益',
  '大器晚成，大音希声，大象无形',
  '知人者智，自知者明；胜人者有力，自胜者强',
  '强行者有志',
  '慎终如始，则无败事',
  '图难于其易，为大于其细',
  '吾生也有涯，而知也无涯',
  '人生天地之间，若白驹之过隙，忽然而已',
  '水之积也不厚，则其负大舟也无力',
  '风之积也不厚，则其负大翼也无力',

  // === 两汉魏晋 · 惜时励志 ===
  '少壮不努力，老大徒伤悲',
  '百川东到海，何时复西归',
  '老骥伏枥，志在千里；烈士暮年，壮心不已',
  '鞠躬尽瘁，死而后已',
  '非淡泊无以明志，非宁静无以致远',
  '夫学须静也，才须学也。非学无以广才，非志无以成学',
  '淫慢则不能励精，险躁则不能治性',
  '年与时驰，意与日去，遂成枯落，多不接世',
  '静以修身，俭以养德',
  '燕雀安知鸿鹄之志哉',
  '王侯将相，宁有种乎',
  '精诚所至，金石为开',
  '绳锯木断，水滴石穿',
  '大禹圣人，乃惜寸阴；至于众人，当惜分阴',
  '近朱者赤，近墨者黑',
  '有志者，事竟成',
  '丈夫志四海，万里犹比邻',
  '及时当勉励，岁月不待人',
  '盛年不重来，一日难再晨',
  '奇文共欣赏，疑义相与析',
  '刑天舞干戚，猛志固常在',
  '悟已往之不谏，知来者之可追',
  '实迷途其未远，觉今是而昨非',
  '不戚戚于贫贱，不汲汲于富贵',

  // === 唐诗 · 进取读书 ===
  '长风破浪会有时，直挂云帆济沧海',
  '天生我材必有用，千金散尽还复来',
  '仰天大笑出门去，我辈岂是蓬蒿人',
  '大鹏一日同风起，扶摇直上九万里',
  '俱怀逸兴壮思飞，欲上青天揽明月',
  '欲穷千里目，更上一层楼',
  '会当凌绝顶，一览众山小',
  '读书破万卷，下笔如有神',
  '业精于勤荒于嬉，行成于思毁于随',
  '书山有路勤为径，学海无涯苦作舟',
  '黑发不知勤学早，白首方悔读书迟',
  '三更灯火五更鸡，正是男儿读书时',
  '青春须早为，岂能长少年',
  '劝君莫惜金缕衣，劝君惜取少年时',
  '花开堪折直须折，莫待无花空折枝',
  '千淘万漉虽辛苦，吹尽狂沙始到金',
  '莫愁前路无知己，天下谁人不识君',
  '试玉要烧三日满，辨材须待七年期',
  '野火烧不尽，春风吹又生',
  '沉舟侧畔千帆过，病树前头万木春',
  '请君暂上凌烟阁，若个书生万户侯',
  '十年磨一剑，霜刃未曾试',
  '两句三年得，一吟双泪流',
  '吟安一个字，捻断数茎须',
  '为人性僻耽佳句，语不惊人死不休',
  '桐花万里丹山路，雏凤清于老凤声',
  '少年心事当拏云，谁念幽寒坐呜呃',
  '我有迷魂招不得，雄鸡一声天下白',
  '男儿何不带吴钩，收取关山五十州',
  '少年辛苦终身事，莫向光阴惰寸功',
  '富贵必从勤苦得，男儿须读五车书',
  '熟读唐诗三百首，不会作诗也会吟',
  '莫道桑榆晚，为霞尚满天',
  '自古逢秋悲寂寥，我言秋日胜春朝',
  '君不见，黄河之水天上来，奔流到海不复回',
  '君不见，高堂明镜悲白发，朝如青丝暮成雪',
  '夫天地者，万物之逆旅也；光阴者，百代之过客也',
  '胜败兵家事不期，包羞忍耻是男儿',

  // === 宋词宋诗 · 治学坚志 ===
  '莫等闲，白了少年头，空悲切',
  '三十功名尘与土，八千里路云和月',
  '待从头、收拾旧山河，朝天阙',
  '了却君王天下事，赢得生前身后名',
  '想当年，金戈铁马，气吞万里如虎',
  '竹杖芒鞋轻胜马，谁怕？一蓑烟雨任平生',
  '回首向来萧瑟处，归去，也无风雨也无晴',
  '博观而约取，厚积而薄发',
  '古之立大事者，不惟有超世之才，亦必有坚忍不拔之志',
  '旧书不厌百回读，熟读深思子自知',
  '粗缯大布裹生涯，腹有诗书气自华',
  '纸上得来终觉浅，绝知此事要躬行',
  '山重水复疑无路，柳暗花明又一村',
  '汝果欲学诗，工夫在诗外',
  '小荷才露尖尖角，早有蜻蜓立上头',
  '等闲识得东风面，万紫千红总是春',
  '问渠那得清如许，为有源头活水来',
  '半亩方塘一鉴开，天光云影共徘徊',
  '读书之法，在循序而渐进，熟读而精思',
  '少年易老学难成，一寸光阴不可轻',
  '未觉池塘春草梦，阶前梧叶已秋声',
  '古来学问无遗力，少壮工夫老始成',
  '昨夜西风凋碧树，独上高楼，望尽天涯路',
  '衣带渐宽终不悔，为伊消得人憔悴',
  '众里寻他千百度，蓦然回首，那人却在灯火阑珊处',
  '不畏浮云遮望眼，自缘身在最高层',
  '看似寻常最奇崛，成如容易却艰辛',
  '生当作人杰，死亦为鬼雄',
  '壮心未与年俱老，死去犹能作鬼雄',
  '人生自古谁无死，留取丹心照汗青',
  '忧劳可以兴国，逸豫可以亡身',
  '青山遮不住，毕竟东流去',
  '横看成岭侧成峰，远近高低各不同',
  '不识庐山真面目，只缘身在此山中',
  '士大夫三日不读书，则义理不交于胸中',
  '随人作计终后人，自成一家始逼真',
  '学如逆水行舟，不进则退',
  '读书百遍，其义自见',

  // === 元明清 · 勤学箴言 ===
  '不要人夸好颜色，只留清气满乾坤',
  '粉骨碎身浑不怕，要留清白在人间',
  '千磨万击还坚劲，任尔东西南北风',
  '落红不是无情物，化作春泥更护花',
  '我劝天公重抖擞，不拘一格降人才',
  '江山代有才人出，各领风骚数百年',
  '咬定青山不放松，立根原在破岩中',
  '明日复明日，明日何其多。我生待明日，万事成蹉跎',
  '天下事有难易乎？为之，则难者亦易矣；不为，则易者亦难矣',
  '人之为学有难易乎？学之，则难者亦易矣；不学，则易者亦难矣',
  '读书好，多读书，读好书',
  '世事洞明皆学问，人情练达即文章',
  '有志者，事竟成，破釜沉舟，百二秦关终属楚',
  '苦心人，天不负，卧薪尝胆，三千越甲可吞吴',
  '海纳百川，有容乃大；壁立千仞，无欲则刚',
  '一万年太久，只争朝夕',
  '雄关漫道真如铁，而今迈步从头越',
  '红军不怕远征难，万水千山只等闲',
  '世上无难事，只要肯登攀',
  '书痴者文必工，艺痴者技必良',
  '少年负壮气，奋烈自有时',
  '玉不琢，不成器；人不学，不知义',

  // === 曾国藩 · 顾炎武 · 梁启超 ===
  '天下兴亡，匹夫有责',
  '君子之学必日新，日新者日进也',
  '读书不二：一书未点完，断不看他书',
  '天下古今之庸人，皆以一惰字致败',
  '勤字功夫，第一贵早起，第二贵有恒',
  '少年智则国智，少年富则国富，少年强则国强',
  '故今日之责任，不在他人，而全在我少年',
  '为学譬如熬肉，先须用猛火煮，然后用慢火温',
  '凡将立大事，必先做小事',
  '养活一团春意思，撑起两根穷骨头',
  '盛年不重来，一日难再晨，及时当勉励，岁月不待人',

  // === 近现代名言 · 励志 ===
  '宝剑锋从磨砺出，梅花香自苦寒来',
  '每一个不曾起舞的日子，都是对生命的辜负',
  '既然选择了远方，便只顾风雨兼程',
  '没有比脚更长的路，没有比人更高的山',
  '如果你因失去了太阳而流泪，那么你也将失去群星了',
  '世界以痛吻我，我要报之以歌',
  '星光不问赶路人，时光不负有心人',
  '乾坤未定，你我皆是黑马',
  '越努力，越幸运',
  '不怕万人阻挡，只怕自己投降',
  '你的坚持，终将美好',
  '坚持就是胜利',
  '你现在的努力，是为了以后有更多选择',
  '今天多一分努力，明天少一句求人',
  '所有的成功，都来自不倦的努力',
  '少年回头望，笑我还不快跟上',
  '埋头是题海，抬头是未来',
  '心之所向，素履以往',
  '但凡辛苦，皆是礼物',
  '你要悄悄拔尖，然后惊艳所有人',
  '半山腰太挤了，去山顶看看吧',
  '放弃不难，但坚持一定很酷',
  '慢慢来，会好的',
  '煎和熬都是变美味的方式，加油也是',
  '你所热爱的东西，有一天会反过来拥抱你',
  '无人问津的日子里，正是登峰造极的好时机',
  '请再悄悄加点油，无论如何都想听你说，我终于成为不负众望的人了',
  '来得及，考得上，你可以',
  '种一棵树最好的时间是十年前，其次是现在',
  '你的气质里，藏着你读过的书和走过的路',
  '生活原本沉闷，但跑起来就有风',
  '唯有父母和前途不可辜负',
  '总不能还没努力就向生活妥协吧',
  '将来的你，一定会感谢现在拼命的自己',
  '只有极其努力，才能看起来毫不费力',
  '努力只能及格，拼命才能优秀',
  '熬过无人问津的日子，才能拥抱诗和远方',
  '哪有什么一夜成名，其实都是百炼成钢',
  '既然做不了富二代，那就努力做富一代吧',
  '最怕你一生碌碌无为，还安慰自己平凡可贵',
  '你不努力，谁也给不了你想要的生活',
  '别让未来的你，讨厌现在不努力的自己',
  '该努力的时候，别选择了安逸',
  '成功的路上并不拥挤，因为坚持的人不多',
  '宁吃百日苦，不留终生憾',
  '没有一份苦会白吃，没有一滴汗会白流',
  '你可以一落千丈，也可以一鸣惊人',
  '弱者才会一蹶不振，强者只会逆风翻盘',
  '既然目标是地平线，留给世界的只能是背影',
  '人之所以能，是因为相信能',
  '所谓的光辉岁月，并不是后来闪耀的日子，而是无人问津时对梦想的偏执',
  '累了可以休息，但不要放弃',
  '真正的大师，永远怀着一颗学徒的心',
  '只要路是对的，就不怕路远',
  '你只管努力，剩下的交给时间',
  '山不过来，我就过去',
  '逆风的方向，更适合飞翔',
  '活在当下，行在今日',

  // === 民间谚语与成语 · 勤学 ===
  '一分耕耘，一分收获',
  '只要功夫深，铁杵磨成针',
  '吃得苦中苦，方为人上人',
  '功夫不负有心人',
  '笨鸟先飞早入林',
  '一勤天下无难事',
  '勤能补拙是良训，一分辛苦一分才',
  '台上一分钟，台下十年功',
  '冰冻三尺，非一日之寒',
  '罗马不是一天建成的',
  '千里之堤，溃于蚁穴',
  '失之毫厘，差之千里',
  '机不可失，时不再来',
  '百闻不如一见',
  '实践出真知',
  '三个臭皮匠，顶个诸葛亮',
  '众人拾柴火焰高',
  '一花独放不是春，百花齐放春满园',
  '良药苦口利于病，忠言逆耳利于行',
  '满招损，谦受益',
  '虚心使人进步，骄傲使人落后',
  '一日之计在于晨，一年之计在于春',
  '一寸光阴一寸金，寸金难买寸光阴',
  '光阴似箭，日月如梭',
  '花有重开日，人无再少年',
  '好记性不如烂笔头',
  '眼过千遍，不如手过一遍',
  '师父领进门，修行在个人',
  '活到老，学到老',
  '学无止境',
  '艺多不压身',
  '行行出状元',
  '若要功夫深，铁杵磨成针',
  '拳不离手，曲不离口',
  '冬练三九，夏练三伏',
  '磨刀不误砍柴工',
  '授人以鱼，不如授人以渔',
  '近水知鱼性，近山识鸟音',
  '宰相必起于州部，猛将必发于卒伍',
  '路遥知马力，日久见人心',
  '疾风知劲草，板荡识诚臣',
  '烈火见真金',
  '真金不怕火炼',
  '是金子总会发光',
  '酒香不怕巷子深',
  '不经一番寒彻骨，怎得梅花扑鼻香',
  '雨后见彩虹',
  '否极泰来，苦尽甘来',
  '车到山前必有路，船到桥头自然直',
  '守得云开见月明',
  '没有过不去的火焰山',
  '先苦后甜，苦尽甘来',
  '风雨过后，必见彩虹',
  '天道酬勤',
  '勤学如春起之苗，不见其增，日有所长',
  '博学之，审问之，慎思之，明辨之，笃行之',

  // === 汪国真 ===
  '我不去想是否能够成功，既然选择了远方，便只顾风雨兼程',
  '如果你是大海，何必在乎别人把你说成小溪',
  '如果你是峰峦，何必在乎别人把你当成平地',
  '如果你是春色，何必为一瓣花朵的凋零叹息',
  '如果你是种子，何必为还没有结出果实着急',
  '无论天上掉下来的是什么，生命总是美丽的',
  '只要青春还在，我就不会悲哀',

  // === 泰戈尔 ===
  '只有经历过地狱般的磨砺，才能练就创造天堂的力量',
  '只有流过血的手指，才能弹出世间的绝唱',

  // === 其他外国名言 ===
  '天才就是百分之一的灵感加上百分之九十九的汗水',
  '我未曾见过一个早起勤奋谨慎诚实的人抱怨命运不好',
  '时间就像海绵里的水，只要愿挤，总还是有的',
  '哪里有天才，我是把别人喝咖啡的工夫都用在工作上的',
  '伟大的成绩和辛勤劳动是成正比例的',
  '不满是向上的车轮',
  '愿中国青年都摆脱冷气，只是向上走',
  '有一分热，发一分光',
  '不必等候炬火，此后如竟没有炬火，我便是唯一的光',
  '真正的勇士，敢于直面惨淡的人生，敢于正视淋漓的鲜血',
  '希望是本无所谓有，无所谓无的。这正如地上的路；其实地上本没有路，走的人多了，也便成了路',
  '猛兽总是独行，牛羊才成群结队',
  '时间就是性命，无端的空耗别人的时间，其实无异于谋财害命',
  '每个人都有一个觉醒期，但觉醒的早晚决定个人的命运',
  '人生就是如此，你以为翻过了一座山，却不过是另一段跋涉的起点',

  // === 域外哲思 · 斯多葛与励志 ===
  '你有力量控制你的心，而不是外在的事物',
  '把全部精力放在你能控制的事情上',
  '一个人的生命价值，在于他贡献了什么，而不是积累了什么',
  '每一天，都是重新开始的机会',
  '如果你不能飞，那就跑；如果你不能跑，那就走；如果你不能走，那就爬——无论如何，你必须一直向前',
  '杀不死我的，使我更强大',
  '时光飞逝；把握现在，把余下的时间献给沉思',
  '生命给了你，不是用来浪费的',
  '唯有在黑暗中，才能看见星光',
  '所谓奇迹，不过是努力的另一个名字',
  '他山之石，可以攻玉',
  '也许你正在承受的，恰恰是在为你塑造',
  '一切过往，皆为序章',

  // === 考研专属 ===
  '你背单词时，阿拉斯加的鳕鱼正跃出水面',
  '你算数学时，太平洋彼岸的海鸥振翅掠过城市上空',
  '你晚自习时，极圈上的夜空散漫了五彩斑斓',
  '当你为自己的未来踏踏实实努力时',
  '那些你感觉从来不会看到的景色',
  '那些你觉得终身不会遇到的人',
  '正一步步向你走来',
  '考研路上，你从来不是一个人',
  '所有的逆袭，都是有备而来',
  '所有的幸运，都是努力埋下的伏笔',
  '能坚持别人不能坚持的，才能拥有别人不能拥有的',
  '别人在睡觉，你在刷题；别人在刷剧，你在背书',
  '这一把，我们都要赢',
  '以梦为马，不负韶华',
  '我们都在努力奔跑，我们都是追梦人',
  '愿你们合上笔盖的那一刻，有武士收刀入鞘般的骄傲',
  '考研是一场孤独的修行，但终点一定繁花似锦',
  '你做的每一道题，都在为未来的自己铺路',
  '过程难熬，结局会好',
  '每一个清晨的早起，都是对梦想的致敬',
  '每一个深夜的坚持，都是对未来的承诺',
  '你不努力，没人替你坚强',
  '熬过的夜，终会变成光',
  '今天所有的努力，都是为了以后有更多选择的权力',
  '坚持到最后一刻，你就已经赢了大多数人',
  '轻舟已过万重山，前路漫漫亦灿灿',
  '追风赶月莫停留，平芜尽处是春山',
  '且将新火试新茶，诗酒趁年华',
  '少年不惧岁月长，彼方尚有荣光在',
  '各自努力，顶峰相见',
  '当你在夜晚孤军奋战时，漫天星光因你而闪烁',
  '等苦尽甘来的那一天，山河星月都做贺礼',
  '虽然辛苦，但我还是会选择那种滚烫的人生',
  '向前走，不要回头，光明就在前方',
  '坚持住，天快亮了',
  '你的努力，终将惊艳时光',
  '把努力当成一种习惯，而不是一时热血',
  '所有的出口成章，都是厚积薄发',
  '所有的从容淡定，都是世事磨砺',
  '所有的游刃有余，都是千锤百炼',
  '不是因为看到希望才坚持，而是因为坚持才看到希望',
  '比起失败，遗憾更让人难以释怀',
  '世界很大，考场很小，人生很长，不慌不忙',
  '考试从来不是人生的终点，但它是你去看世界的路',
  '永远相信美好的事情即将发生',
  '追光的人，终会光芒万丈',
  '你的名字这么好听，一定要出现在录取通知书上',
  '愿你上岸，阳光万里',
  '一研为定，明年研一',
];

// Fisher-Yates shuffle
function shuffle(arr) {
  const a = [...arr];
  for (let i = a.length - 1; i > 0; i--) {
    const j = Math.floor(Math.random() * (i + 1));
    [a[i], a[j]] = [a[j], a[i]];
  }
  return a;
}

let shuffled = shuffle(QUOTES);
let index = 0;

export function nextQuote() {
  if (index >= shuffled.length) { shuffled = shuffle(QUOTES); index = 0; }
  return shuffled[index++];
}

// Returns the same quote all day, regardless of how many times called or component remounts
export function getDailyQuote() {
  const d = new Date();
  const key = d.getFullYear() * 10000 + (d.getMonth() + 1) * 100 + d.getDate();
  let h = key;
  h = ((h >> 16) ^ h) * 0x45d9f3b | 0;
  h = ((h >> 16) ^ h) * 0x45d9f3b | 0;
  h = (h >> 16) ^ h;
  return QUOTES[Math.abs(h) % QUOTES.length];
}

export const TOTAL_QUOTES = QUOTES.length;
